...
class JSObject {
  ...
  Map obj2Map(JSObject object) {
    var result = _obj2Map(object, "");
    return result;
  }

  Map _obj2Map(JSObject object, String name) {
    var result = {};
    var property = object.copyPropertyNames();
    for (var i = 0; i < property.count; i++) {
      var name = property.propertyNameArrayGetNameAtIndex(i);
      var v = object.getProperty(name);
      if (!v.isObject) {
        if (v.isNumber) {
          result[name] = v.toNumber();
        } else if (v.isBoolean) {
          result[name] = v.toBoolean;
        } else {
        ...
        if (v.isArray) {
          result[name] = obj2List(v.toObject());
        } else {
          result[name] = _obj2Map(v.toObject(), name);
        }
      }
    }
    return result;
  ...
  List toList(){
    return obj2List(this);
  }

  List obj2List(JSObject object) {
    var result = [];
    var property = object.copyPropertyNames();
    for (var i = 0; i < property.count; i++) {
      var name = property.propertyNameArrayGetNameAtIndex(i);
      ...
      if (!v.isObject) {
        ...
        if (v.isArray) {
          result.add(obj2List(v.toObject()));
        } else {
          result.add(_obj2Map(v.toObject(), ""));
        }
      }
    }
    return result;
