'use strict';

var doric = require('doric');

var colors = [
    "#70a1ff",
    "#7bed9f",
    "#ff6b81",
    "#a4b0be",
    "#f0932b",
    "#eb4d4b",
    "#6ab04c",
    "#e056fd",
    "#686de0",
    "#30336b" ].map(function (e) { return doric.Color.parse(e); });
function label(str) {
    return doric.text({
        text: str,
        textSize: 16,
    });
}
function title(str) {
    return doric.text({
        text: str,
        layoutConfig: doric.layoutConfig().configWidth(doric.LayoutSpec.MOST),
        textSize: 30,
        textColor: doric.Color.WHITE,
        backgroundColor: colors[1],
        textAlignment: doric.gravity().center(),
        height: 50,
    });
}

var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") { r = Reflect.decorate(decorators, target, key, desc); }
    else { for (var i = decorators.length - 1; i >= 0; i--) { if (d = decorators[i]) { r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r; } } }
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var shareIcon = "data:image/png;base64,";
var NavbarDemo = /*@__PURE__*/(function (Panel) {
    function NavbarDemo () {
        Panel.apply(this, arguments);
    }

    if ( Panel ) NavbarDemo.__proto__ = Panel;
    NavbarDemo.prototype = Object.create( Panel && Panel.prototype );
    NavbarDemo.prototype.constructor = NavbarDemo;

    NavbarDemo.prototype.build = function build (rootView) {
        doric.scroller(doric.vlayout([
            title("Navbar Demo"),
            label('isHidden').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).isHidden().then(function (e) { return doric.modal(context).alert(("Navbar isHidden:" + e)); }).catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setHidden').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).isHidden()
                        .then(function (e) { return doric.navbar(context).setHidden(!e); })
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setTitle').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).setTitle('Setted Title')
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setBgColor').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).setBgColor(doric.Color.YELLOW)
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setLeft').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).setLeft(doric.text({
                        width: 70,
                        height: 44,
                        textColor: doric.Color.BLACK,
                        layoutConfig: doric.layoutConfig().just().configAlignment(doric.Gravity.Center),
                        text: "Left",
                    }))
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setRight').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).setRight(doric.hlayout([
                        doric.text({
                            width: 70,
                            height: 44,
                            textColor: doric.Color.BLACK,
                            layoutConfig: doric.layoutConfig().just().configAlignment(doric.Gravity.Center),
                            text: "Right",
                        }),
                        doric.image({
                            imageBase64: shareIcon
                        }).apply({
                            layoutConfig: doric.layoutConfig().just().configAlignment(doric.gravity().centerY()),
                            width: 33,
                            height: 33,
                            onClick: function () {
                                doric.modal(context).toast('Right Clicked', doric.Gravity.Bottom);
                            }
                        })
                    ]))
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('setCenter').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navbar(context).setCenter(doric.hlayout([
                        doric.text({
                            width: 70,
                            height: 44,
                            textColor: doric.Color.BLACK,
                            layoutConfig: doric.layoutConfig().just().configAlignment(doric.Gravity.Center),
                            text: "Center",
                        }) ]).apply({
                        backgroundColor: doric.Color.RED
                    }))
                        .catch(function (e) {
                        doric.modal(context).alert(e);
                    });
                }
            }),
            label('Pop').apply({
                width: 200,
                height: 50,
                backgroundColor: colors[0],
                textSize: 30,
                textColor: doric.Color.WHITE,
                layoutConfig: doric.layoutConfig().just(),
                onClick: function () {
                    doric.navigator(context).pop();
                }
            }) ], {
            layoutConfig: doric.layoutConfig().most().configHeight(doric.LayoutSpec.FIT),
            gravity: doric.gravity().center(),
            space: 10,
        }), {
            layoutConfig: doric.layoutConfig().most(),
            backgroundColor: doric.Color.BLUE,
        }).in(rootView);
    };

    return NavbarDemo;
}(doric.Panel));
NavbarDemo = __decorate([
    Entry
], NavbarDemo);
//# sourceMappingURL=NavbarDemo.es5.js.map
