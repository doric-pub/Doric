    ...
    build(rootView) {
        let pager = doric.slider({
            ...
            renderPage: (idx) => {
                return doric.slideItem(doric.image({
                    ...
                    layoutConfig: doric.layoutConfig().configWidth(doric.LayoutSpec.MOST).configHeight(doric.LayoutSpec.FIT).configAlignment(doric.gravity().center()),
                })).also(it => {
                    let start = idx;
                    it.onClick = () => {
        ...
        rootView.addChild(doric.vlayout([
            doric.text({
                ...
                onClick:()=>{
                 pager.slidePage(context, 2, false);
                },
                textSize: 30,
