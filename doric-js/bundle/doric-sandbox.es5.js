	...
	*/
	...
	_export({ target: 'Map', stat: true, forced: DOES_NOT_WORK_WITH_PRIMITIVES$1 }, {
	  groupBy: function groupBy(items, callbackfn) {
	    ...
	    iterate(items, function (value) {
	      ...
	      if (!has$g(map, key)) { set$d(map, key, [value]); }
	...
	_export({ target: 'Math', stat: true }, {
	  cbrt: function cbrt(x) {
	    var n = +x;
	    return mathSign(n) * pow$7(abs$6(n), 1 / 3);
	  }
	});
	...
	var pow$5 = function (x, n, acc) {
	  return n === 0 ? acc : n % 2 === 1 ? pow$5(x, n - 1, acc * x) : pow$5(x * x, n / 2, acc);
	};
	...
	_export({ target: 'Number', proto: true, forced: FORCED$e }, {
	  toFixed: function toFixed(fractionDigits) {
	    ...
	    if (number > 1e-21) {
	      e = log$1(number * pow$5(2, 69, 1)) - 69;
	...
	// https://tc39.es/ecma262/#sec-get-regexp.prototype.flags
	if (FORCED$7) { defineBuiltInAccessor(RegExpPrototype$4, 'flags', {
	  configurable: true,
	  get: regexpFlags
	}); }
	...
	var MIN_SUBNORMAL16 = pow$4(2, -24); // 2 ** -10 * 2 ** -14
	...
	var unpackFloat16 = function (bytes) {
	  ...
	  return pow$4(2, exponent - 15) * (sign === 0 ? 1 + significand * SIGNIFICAND_DENOM16 : -1 - significand * SIGNIFICAND_DENOM16);
	};
	...
	var packFloat16 = function (value) {
	  ...
	  var significand = mathRoundTiesToEven((value * pow$3(2, -exponent) - 1) * REC_SIGNIFICAND_DENOM16);
	...
	var FORCED$2 = !$isError || !PROTOTYPE_SETTING_AVAILABLE || fails(function () {
	...
	_export({ target: 'Error', stat: true, sham: true, forced: FORCED$2 }, {
	...
	_export({ target: 'RegExp', stat: true, forced: FORCED$1 }, {
	...
	// https://github.com/tc39/proposal-arraybuffer-base64
	if (Uint8Array$1) { _export({ target: 'Uint8Array', proto: true }, {
	...
	_export({ target: 'WeakMap', proto: true, real: true, forced: true }, {
	  deleteAll: function deleteAll(/* ...elements */) {
	    ...
	    var wasDeleted;
	    for (var k = 0, len = arguments.length; k < len; k++) {
	      wasDeleted = remove$1(collection, arguments$1[k]);
	      allDeleted = allDeleted && wasDeleted;
	    } return !!allDeleted;
	  }
	});
