var doric = (function (exports) {
	...
	var classofRaw = function (it) {
	  return stringSlice$g(toString$2(it), 8, -1);
	};
	...
	*/
	...
	var mathLog1p = Math.log1p || function log1p(x) {
	  return (x = +x) > -1e-8 && x < 1e-8 ? x - x * x / 2 : log$8(1 + x);
	};
	...
	function asinh(x) {
	  return !isFinite(x = +x) || x == 0 ? x : x < 0 ? -asinh(-x) : log$6(x + sqrt$1(x * x + 1));
	}
	...
	_export({ target: 'Math', stat: true }, {
	  clz32: function clz32(x) {
	    return (x >>>= 0) ? 31 - floor$7(log$4(x + 0.5) * LOG2E) : 32;
	  }
	});
